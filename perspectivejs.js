...
var Perspective = function(ctxd, image) {
    // check the arguments
    if( ! ctxd || ! ctxd.strokeStyle ) { return; }
    if( ! image || ! image.width || ! image.height ) { return; }
    // prepare a <canvas> for the image
    var cvso = createCanvas(image.width, image.height);
    var ctxo = cvso.getContext('2d');
    ctxo.drawImage(image, 0, 0, cvso.width, cvso.height);
    // prepare a <canvas> for the transformed image
    var cvst = createCanvas(ctxd.canvas.width, ctxd.canvas.height);
...
Perspective.prototype.draw = function(points) {
    var d0x = points[0][0];
    var d0y = points[0][1];
    var d1x = points[1][0];
    var d1y = points[1][1];
...
Perspective.prototype._applyClipPath = function(ctx, points) {
    ctx.beginPath();
